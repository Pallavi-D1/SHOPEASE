  ...
  return (
    <div className="border-b border-gray-200 sticky top-0 z-50 bg-white/95 backdrop-blur-sm shadow-sm">
      <Container className="py-4 lg:py-6 flex items-center gap-x-3 md:gap-x-7 justify-between">
        ...
        <div className="hidden md:inline-flex items-center gap-4 lg:gap-x-6 text-sm uppercase font-medium text-[#ef476f]">
          {headerNavigation.map((item) => (
            <NavLink
              key={item?.title}
              className={`hover:text-[#ef476f] duration-300 relative group overflow-hidden px-1 py-2 transition-colors ${
            ...
            >
              <div className="relative flex items-center">
                ...
                {item?.title === "Orders" && userInfo && orderCount > 0 && (
                  <span className="absolute -right-1 -top-2 w-4 h-4 rounded-full text-xs bg-[#ef476f] text-white flex items-center justify-center font-medium animate-pulse">
                    {orderCount}
                  </span>
                )}
              </div>
              <span
                className={`absolute bottom-0 left-0 inline-block w-full h-0.5 bg-[#ef476f] group-hover:translate-x-0 duration-300 ease-out ${
                  location?.pathname === item?.link
                    ? "translate-x-0"
                    : "-translate-x-full"
                }`}
          ...
          <NavLink
            to="/cart"
            className={({ isActive }) =>
              `text-2xl relative transition-colors duration-300 p-2 ${
                isActive ? "text-[#ef476f]" : "text-gray-700 hover:text-[#ef476f]"
              }`
            }
          >
            <IoMdCart />
            {products?.length > 0 && (
              <span className="absolute -right-1 -top-1 w-5 h-5 rounded-full text-xs bg-[#ef476f] text-white flex items-center justify-center font-medium animate-pulse">
                {products.length}
              </span>
            )}
          </NavLink>
              ...
              <div className="p-6">
                ...
                <div className="space-y-1">
                    ...
                    >
                      <div className="flex items-center gap-3 justify-between">
                        ...
                        {item?.title === "Orders" &&
                          userInfo &&
                          orderCount > 0 && (
                            <span className="bg-[#ef476f] text-white text-xs px-2 py-1 rounded-ful">
                              {orderCount}
                            </span>
                          )}
                      </div>
                  ...
                  <NavLink
                    ...
                    onClick={() => setIsOpen(false)}
                    className={({ isActive }) =>
                      `block px-4 py-3 rounded-lg transition-all duration-200 transform hover:translate-x-1 ${
                        isActive
                          ? "bg-gray-100 text-[#ef476f] font-semibold"
