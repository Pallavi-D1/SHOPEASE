...
const Cart = () => {
  ...
  const fetchAddresses = async () => {
    try {
      ...
      const response = await fetch("https://shopease-backend-pxin.onrender.com/api/user/addresses", {
        headers: {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch("https://shopease-backend-pxin.onrender.com/api/order/create", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-lg border border-gray-200 p-6 sticky top-8">
                ...
                <button
                  ...
                  className="w-full bg-[#ef476f] text-white py-4 px-6 rounded-md hover:bg-[#d63f63] transition-colors font-medium text-lg disabled:opacity-50 disabled:cursor-not-allowed
"
                >
                  {!userInfo ? (
